...
:if ([:len $content] = 0) do={
    :log info "[VENDAS] Nenhuma venda recente encontrada"
    :do { /file remove [find name="vendas.txt"] } on-error={}
    return
}

:log info ("[VENDAS] Processando vendas recentes...")
...
:log info ("[VENDAS] Encontradas $totalVendas vendas para processar")
