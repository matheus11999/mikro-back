...
:local notificar do={
    ...
    :do {
        ...
        :log warning "Falha na notificacao: $acao para $mac"
    }
}
...
:local cmdNotificar "/tool fetch url=\"$authUrl\" http-method=post http-header-field=\"Content-Type: application/json\" http-data=\"{\\\"token\\\":\\\"$authToken\\\",\\\"mac_address\\\":\\\"$mac\\\",\\\"mikrotik_id\\\":\\\"$mikrotikId\\\",\\\"action\\\":\\\"disconnect\\\"}\""
:local cmdLimpar "/system scheduler remove [find name=\"$nomeScheduler\"]"
:local cmdCompleto "$cmdRemover; $cmdNotificar; $cmdLimpar"

/system scheduler add name=$nomeScheduler start-time=$tempo interval=0 on-event=$cmdCompleto comment="PIX-EXPIRE-$mac"
