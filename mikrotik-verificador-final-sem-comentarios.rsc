...
:local macs ""
:for tentativa from=1 to=5 do={
    :log info "Tentativa $tentativa"
...
:if ([:len $macs] > 0) do={
    ...
    :log info "Executando notificador..."
    /system script run notificador-pix
} else={
    :log info "Nenhum MAC processado"
}
:log info "PIX concluido" 